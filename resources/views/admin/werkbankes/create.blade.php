...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.werkbanke.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.werkbankes.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="werkbank">{{ trans('cruds.werkbanke.fields.werkbank') }}</label>
                <input class="form-control {{ $errors->has('werkbank') ? 'is-invalid' : '' }}" type="text" name="werkbank" id="werkbank" value="{{ old('werkbank', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('endergebnis') ? 'is-invalid' : '' }}" name="endergebnis_id" id="endergebnis_id">
                    @foreach($endergebnis as $id => $entry)
                        <option value="{{ $id }}" {{ old('endergebnis_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bauteile') ? 'is-invalid' : '' }}" type="text" name="bauteile" id="bauteile" value="{{ old('bauteile', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('xp') ? 'is-invalid' : '' }}" type="text" name="xp" id="xp" value="{{ old('xp', '') }}">
                @if($errors->has('xp'))
                    <div class="invalid-feedback">
                        {{ $errors->first('xp') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lvl') ? 'is-invalid' : '' }}" type="text" name="lvl" id="lvl" value="{{ old('lvl', '') }}">
                @if($errors->has('lvl'))
                    <div class="invalid-feedback">
                        {{ $errors->first('lvl') }}
                    </div>
