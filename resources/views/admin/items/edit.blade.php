    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.items.update", [$item->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('item_name') ? 'is-invalid' : '' }}" type="text" name="item_name" id="item_name" value="{{ old('item_name', $item->item_name) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('spawn_name') ? 'is-invalid' : '' }}" type="text" name="spawn_name" id="spawn_name" value="{{ old('spawn_name', $item->spawn_name) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('gewicht') ? 'is-invalid' : '' }}" type="text" name="gewicht" id="gewicht" value="{{ old('gewicht', $item->gewicht) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('seltenes_item') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="seltenes_item" id="seltenes_item" value="1" {{ $item->seltenes_item || old('seltenes_item', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kategorie') ? 'is-invalid' : '' }}" name="kategorie_id" id="kategorie_id">
                    @foreach($kategories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kategorie_id') ? old('kategorie_id') : $item->kategorie->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
