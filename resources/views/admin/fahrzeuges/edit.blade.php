...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.fahrzeuge.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.fahrzeuges.update", [$fahrzeuge->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fahrzeug_name') ? 'is-invalid' : '' }}" type="text" name="fahrzeug_name" id="fahrzeug_name" value="{{ old('fahrzeug_name', $fahrzeuge->fahrzeug_name) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('spawn_name') ? 'is-invalid' : '' }}" type="text" name="spawn_name" id="spawn_name" value="{{ old('spawn_name', $fahrzeuge->spawn_name) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('preis') ? 'is-invalid' : '' }}" type="text" name="preis" id="preis" value="{{ old('preis', $fahrzeuge->preis) }}">
                @if($errors->has('preis'))
                    <div class="invalid-feedback">
                        {{ $errors->first('preis') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('kofferraum_groesse') ? 'is-invalid' : '' }}" type="text" name="kofferraum_groesse" id="kofferraum_groesse" value="{{ old('kofferraum_groesse', $fahrzeuge->kofferraum_groesse) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('herstellbar') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="herstellbar" id="herstellbar" value="1" {{ $fahrzeuge->herstellbar || old('herstellbar', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('im_shop') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="im_shop" id="im_shop" value="1" {{ $fahrzeuge->im_shop || old('im_shop', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kategories') ? 'is-invalid' : '' }}" name="kategories[]" id="kategories" multiple>
                    @foreach($kategories as $id => $kategory)
                        <option value="{{ $id }}" {{ (in_array($id, old('kategories', [])) || $fahrzeuge->kategories->contains($id)) ? 'selected' : '' }}>{{ $kategory }}</option>
                    @endforeach
                </select>
