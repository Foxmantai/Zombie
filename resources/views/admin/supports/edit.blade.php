    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.supports.update", [$support->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('titel') ? 'is-invalid' : '' }}" type="text" name="titel" id="titel" value="{{ old('titel', $support->titel) }}" required>
                @if($errors->has('titel'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ingame_name') ? 'is-invalid' : '' }}" type="text" name="ingame_name" id="ingame_name" value="{{ old('ingame_name', $support->ingame_name) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('grund') ? 'is-invalid' : '' }}" name="grund" id="grund" required>{{ old('grund', $support->grund) }}</textarea>
                @if($errors->has('grund'))
                    <div class="invalid-feedback">
                        {{ $errors->first('grund') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('supporter') ? 'is-invalid' : '' }}" type="text" name="supporter" id="supporter" value="{{ old('supporter', $support->supporter) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\Support::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $support->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('kategorie') ? 'is-invalid' : '' }}" name="kategorie" id="kategorie" required>
                    ...
                    @foreach(App\Models\Support::KATEGORIE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('kategorie', $support->kategorie) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
